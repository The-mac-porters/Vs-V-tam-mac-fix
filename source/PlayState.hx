...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (curStage == 'nogames') 
			camFollow.setPosition(BACK4NGAMES.getMidpoint().x,BACK4NGAMES.getMidpoint().y);
		else 
			...
			for (value in introAssets.keys())
			{
				if (value == curStage)
				{
					...
					if (curStage.contains('school'))
						altSuffix = '-pixel';
					else if (curStage=='bonus'||curStage=='chantown')
						altSuffix = '';
				}
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null && inScene == false)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && inScene == false)
			{
				...
				#end
				if(inScene == false){
				if (curStage =='bonus'){camFollow.setPosition(boyfriend.getMidpoint().x -450, boyfriend.getMidpoint().y - 165); } //doing this cuz why not 
				else if(curStage =='nogames'){camFollow.setPosition(BACK4NGAMES.getMidpoint().x,BACK4NGAMES.getMidpoint().y);}
				else {camFollow.setPosition(dad.getMidpoint().x + 150 + offsetX, dad.getMidpoint().y - 100 + offsetY);}		
				}
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100 && inScene == false)
			{
				...
				#end
				if(inScene == false){
				if (curStage =='bonus'){camFollow.setPosition(boyfriend.getMidpoint().x - 370, boyfriend.getMidpoint().y - 165);}
				else if(curStage =='nogames'){camFollow.setPosition(BACK4NGAMES.getMidpoint().x,BACK4NGAMES.getMidpoint().y);}
				else {camFollow.setPosition(boyfriend.getMidpoint().x - 100 + offsetX, boyfriend.getMidpoint().y - 100 + offsetY);}
				}
				#if windows
				if (luaModchart != null)
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (!(curStep >= 1783 && curStep <= 1791  && curSong =="infinigger"))
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (curSong == "harmony")
						...
						else if  (curSong == "infinigger")
							{
								FlxG.save.data.nogames = true;
